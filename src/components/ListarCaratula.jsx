...
const ListarCaratula = ({ ordenes, onSet }) => {
  ...
  const getAll = async () => {
    const response = await axios.get(`${process.env.REACT_APP_SECOND_API}`);
    onSet(response.data);
  };
    ...
    <>
      ...
      <TableContainer component={Paper} style={{ marginBottom: "40px" }}>
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
          <TableHead>
            <TableRow>
              <StyledTableCell>Caratula</StyledTableCell>
          ...
          <TableBody>
            {ordenes.map((orden) => (
              <StyledTableRow key={orden.caratula}>
